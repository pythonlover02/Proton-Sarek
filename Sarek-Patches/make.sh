...
cp -r "$SCRIPT_DIR/dxvk/x32/"* "$protondir/files/lib/wine/dxvk/i386-windows/" || { echo "Failed to copy 32-bit dxvk files."; exit 1; }

echo "Copying dxvk 64-bit files..."
cp -r "$SCRIPT_DIR/dxvk/x64/"* "$protondir/files/lib/wine/dxvk/x86_64-windows/" || { echo "Failed to copy 64-bit dxvk files."; exit 1; }
